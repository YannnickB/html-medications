 ...
 * 
 */
class Configs extends PdoObjectRecordAbstract  {

	/** Table name */
	protected string $tableName = "Configs";
	/** Primary key column name */
	protected ?string $primaryKeyColumnName = "ConfigId";
	/** Insertable column name */
	protected ?array $insertableColumnsName = [ "ConfigCategory", "ConfigName", "ConfigValue" ];
	/** Updatable column name */
	protected ?array $updatableColumnsName = [ "ConfigValue" ];
...
";
			...
			else $sql .= "ORDER BY ConfigCategory, ConfigName";
			...
			if ( $query ){
				...
				foreach ( $rows as $key => &$row){
					//$row["IllustrationUrl"] = $config["rootUrl"] . "/info/img/" . $row["ConfigIllustrationRelativePath"];
				}
	...
	function fetch():?array {
		global $config;
		$data = parent::fetch();
		//$data["IllustrationUrl"] = $config["rootUrl"] . "/info/img/" . $data["ConfigIllustrationRelativePath"];
		return $data;
	}
}
